...
export default function ResultsPage() {
  ...
  const result = {
    ...
    imageUrl,
    metrics: mockResult.metrics, // Model metrikleri sabit kalabilir
  };

  const metricsData = [
    { name: "Precision", value: result.metrics.precision * 100 },
    { name: "Recall", value: result.metrics.recall * 100 },
    { name: "F1", value: result.metrics.f1 * 100 },
  ];
    ...
    // Orta: Yüklenen görsel
    {
      customContent: (
        <div className="flex items-center justify-center h-full w-full">
          <img src={result.imageUrl} alt="Yüklenen görsel" className="rounded-xl max-h-[80%] max-w-[80%] object-contain shadow-lg" />
        </div>
      ),
    ...
    // Sağ üst: Model metrikleri
    {
      customContent: (
        <div className="flex flex-col items-center justify-center h-full w-full gap-4">
          ...
          <span className="px-3 py-1 rounded-full bg-muted text-foreground font-bold">%{(result.metrics.accuracy * 100).toFixed(2)}</span>
          <div className="text-sm text-muted-foreground">Toplam test: {result.metrics.total}</div>
        </div>
      ),
